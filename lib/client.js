...
function ClusterClient(netAddressArray, redisOptions, poolOptions) {
    ...
    this.readSlave = redisOptions.readSlave || 0;
    delete this.redisOptions.readSlave;
     ...
     */
    this.getPoolOfSlot = function(readOnly, slot) {
        return this.slotProxies.getBySlot(slot).getPool(readOnly);  
    };
    ...
    findClusterDefinition(this.redisNodes, function(err, entryHost, definitions) {
        ...
        for(var i = 0; i < this.numberOfSlots; i++) {
            ...
            (function(ctx, next) {
                proxy.once('ready', function() {
                    ...
                    if(ctx.numberOfReady == ctx.numberOfSlots) {
                        ctx.pubsub.setPool(ctx.getPool(false, 'PUBSUB'));
            ...
            for(var nodeIndex = 2; nodeIndex < slotDef.length; nodeIndex++) {
                ...
                if(nodeIndex === 2) {
                    proxy.setMaster({host: node[0], port: +node[1]});
                } else {
                    slaves.push({host: node[0], port: +node[1]});
                }
            }
            
            proxy.setSlaves(slaves);
 ...
 *
 * @param {Object} err
 * @param {Object} slotProxy
 * @param {Boolean} readOnly
 * @param {Function} done(true if need retry query, otherwise false)
 */
ClusterClient.prototype.handle_exception = function(err, slotProxy, readOnly, sendCommandFunc, next) {
 ...
 */
ClusterClient.prototype.getSlotClient = function(readOnly, key, callback) {
    var pool = this.getPool(readOnly, key);
...
function bindCommand(self, cmd) {
    ClusterClient.prototype[cmd] = function () {
        ...
        var readOnly = this.isReadOnly(cmd);
        var argsAndFn = extractArgsAndCallback(arguments);
        var args = argsAndFn[0];
        var callbackFunc = argsAndFn[1];
        ...
        try {
            ...
            if(!slotProxy) {
                callbackFunc(new Error('no slot serving for key(s) ' + keys));
                return;
            }
            ...
            var _pool = slotProxy.getPool(readOnly);
            
            if(!_pool) {
                callbackFunc(new Error('slot proxy of [' + slotProxy.start + ',' + slotProxy.end + '] do not have any available connections'));
                return;
            }

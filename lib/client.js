...
ClusterClient.prototype.handle_exception = function(err, slotProxy, done) {
    ...
    if (err.message.length > 5) {
        ...
        if(!redirectInfo) {
            ...
            connRefused = err.message.match(/Redis connection to ([^:]+):(\d+) failed - connect ECONNREFUSED/);
        }
    }
    ...
    } else if(connRefused) { // Unavailable
        slotProxy.reset(function(done) {
            return findClusterDefinition(this.redisNodes, done);
        }, function(success) {
            done(success);
        });
    } else { // Others. Do not retry, but raise error event.
        done(false);
    }
};
...
function init_cmd(self, cmd) {
    ClusterClient.prototype[cmd] = function () {
        ...
        try {
            var slotProxy = self.getSlotProxy(keys);
            if(!slotProxy) {
            ...
            _pool.acquire(function (err, client) {
                client.send_command(cmd, argsAndFn[0], function (err, res) {
                    ...
                    self.handle_exception(err, slotProxy, function(retry) {

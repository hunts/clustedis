...
function ClusterClient(netAddressArray, redisOptions, poolOptions) {
    ...
    this.ready = false;
    this.closing = false;
    this.redisServers = netAddressArray;
    ...
    this.getPool = function(keys) {
        ...
        for(var k=0; k<keys.length; k++) {
            ...
            var tmpSlot = this.keySlotMap.hasOwnProperty(keys[k])
                ? this.keySlotMap[keys[k]]
                : slotProxy.calculateSlot(keys[k]);
...
ClusterClient.prototype.init_slot_proxy = function (done) {
    if(!this.redisServers || this.redisServers.length === 0) {
    ...
    var _client = redis.createClient(this.redisServers[0].port, this.redisServers[0].host);

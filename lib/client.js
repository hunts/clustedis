...
function bindCommand(self, cmd) {
    ClusterClient.prototype[cmd] = function () {
        ...
        if(!shardProxy) {
            done(new Error(`no shard serving for key(s) ' ${keys}`));
            return;
        }
        ...
        if(!client || !client.connected) {
            done(new Error(`shard proxy of ['${shardProxy.start},${shardProxy.end}'] do not have any available connection`));
            return;
        }
            ...
            if (!!err) {
                ...
                self.handle_exception(
                    ...
                    function(newClient) { // closure function for replaying current command.
                        newClient.send_command(cmd, args, done);
                    },
                    function() {
                        if (done) {
                            done(err, res);
